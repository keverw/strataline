...
- A prefixed child logger no longer calls a `warn` that is not there. `Logger` declares the method, but a JavaScript caller can supply an object without it, and wrapping such a logger produced a child that had `warn` and reached the one that did not. Prefixed children now fall back to `info`, so a child is never worse than what it wraps.
