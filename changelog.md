...
- Added `getLifecycleState()`, returning `"stopped"`, `"starting"`, `"running"`, `"stopping"`, or `"unstoppable"` as `DevDBLifecycleState`. The refusals above throw, and a call that throws is not a way to find out, so a host with more than one route into a stop had no way to tell "a shutdown is already running" from "a start is, and this call is about to throw". It also tells a joined `stop()` whether it stopped the server or waited on somebody else's shutdown, which the joined promise cannot say, and names the state a failed `start()` leaves when the partially started server outlived even `SIGKILL`, which the child's own exit status cannot distinguish from a healthy or an absent one. It reports this instance's own view, decided the same way the guards are, and says nothing about a server left by a previous run, which remains `getLocalDevDBServerStatus()`'s question.
